...
| **Integridade da instalação** | O `mgr doctor` roda uma **lista fechada de nove verificações** sobre a sua instalação — skill órfã, skill declarada e ausente, agente declarado sem arquivo, arquitetura escolhida sem a skill dela, corpo divergente da fonte do pacote, token `{{MGR_*}}` não resolvido, manifesto atrás do pacote, hook apontando para binário que sumiu, e divergência contra o lockfile. Cada achado traz **arquivo, esperado, encontrado** e a remediação quando ela existe. Ele **nunca escreve**, e não tem `--fix`: cinco das seis correções eram rodar o `mgr update`, que já pede confirmação, e um `--fix` que o chamasse passaria o `-y` por você. Ele **não atesta integridade** — nenhum achado significa *"nenhuma das nove apareceu"*, nunca *"está íntegro"* — e não diz **por que** o defeito aconteceu. Manifesto atrás do pacote é **aviso**, não falha: é o estado normal de quem ainda não rodou `mgr update`. |
