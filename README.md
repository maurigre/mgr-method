...
| **Installation integrity** | `mgr doctor` runs a **closed list of nine checks** over your installation — orphan skill, declared skill missing, declared agent with no file, chosen architecture without its skill, body diverging from the package source, unresolved `{{MGR_*}}` token, manifest behind the package, hook pointing at a binary that is gone, and drift against the lockfile. Every finding carries **file, expected, found** and the remedy when one exists. It **never writes**, and there is no `--fix`: five of the six remedies were running `mgr update`, which already asks for confirmation, and a `--fix` calling it would pass `-y` on your behalf. It **does not attest integrity** — no finding means *"none of the nine appeared"*, never *"this install is sound"* — and it does not say **why** a defect happened. Manifest behind the package is a **warning**, not a failure: it is the normal state of anyone who has not run `mgr update` yet. |
