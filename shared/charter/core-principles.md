...
**Case.** The user picks an architecture at install time and its skill is installed, and
`spec-init` turns that skill's invariants into citable rules in the project's review guide. The
reviewer has them and uses them. But architecture is absent from the five premises of
`spec-execute` - `grep` over that skill returns zero mentions - so the executor is never told to
apply the architecture the user chose. One side of the demand is armed and the other is not.

**Provenance.** `[author]` 2026-09-20, corrected 2026-09-21 (see the amendment below).

> **Amendment of 2026-09-21 - the first Case was wrong about which side was broken.**
>
> As first written, this Case claimed there was "not a single citable `ARCH-*` rule in the review
> guide" and concluded that "the reviewer who sees the violation is forbidden to reprove it, so the
> choice the user made is enforced by nobody". The literal part was true and the conclusion was
> **false**.
>
> Measured on 2026-09-21: the architecture rules exist and are citable - they are named `INV-*`, not
> `ARCH-*`. In this repository the guide carries `INV-1` to `INV-6` (layers, top-down dependency,
> strict layering, separation of responsibilities, logic belongs to the core), generated by
> `spec-init` from the six invariants of the `arch-layered` skill. The review agent reads that
> guide by its first instruction, and in the two isolated reviews of 2026-09-20 it **cited `INV-2`,
> `INV-5` and `INV-6`**.
>
> **How the error was made:** the search enumerated rule prefixes - `DES`, `TST`, `DOC`, `QUAL`,
> `LOG`, `NAM`, `MUT` - and `INV` was not in the list. The enumeration came from what had already
> been seen instead of from the source. `L2.6` requires enumerating a closed set rather than
> approximating it; enumerating only helps when the enumeration itself is complete.
>
> **What survives:** the Statement, unchanged, and still violated - on the executor side only. The
> gap is real and narrower than the first Case described.
